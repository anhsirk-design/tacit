...
export class CodeIndexer {
  ...
  async index(opts?: { maxFiles?: number }): Promise<IndexStats> {
    ...
    for (const abs of files) {
      ...
      if (!lang) { stats.skipped.push({ path: rel, reason: "unsupported language" }); continue; }

      const parsed = await parseFile(rel, source);
      if (!parsed) { stats.skipped.push({ path: rel, reason: "parse unavailable" }); continue; }
    ...
    for (const p of known.keys()) {
      if (!seen.has(p)) { this.store.deleteFile(p); stats.filesRemoved++; }
    }
